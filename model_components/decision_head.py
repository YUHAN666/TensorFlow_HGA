...
def decision_head(x, y, class_num, scope, keep_dropout_head, training, data_format, momentum, mode, reuse=None,
                  drop_rate=0.2, activation='relu'):
    with tf.variable_scope(scope, reuse=reuse):
        channel_axis = 1 if data_format == 'channels_first' else -1

        x = tf.concat([x, y], axis=channel_axis)
        x = CBR(x, 16, 3, 2, training, momentum, mode, name='conv1', padding='same', data_format=data_format,
                activation=activation, bn=True)
        x = CBR(x, 16, 3, 1, training, momentum, mode, name='conv2', padding='same', data_format=data_format,
                activation=activation, bn=True)
        x = CBR(x, 32, 3, 2, training, momentum, mode, name='conv3', padding='same', data_format=data_format,
                activation=activation, bn=True)
        x = CBR(x, 32, 3, 1, training, momentum, mode, name='conv4', padding='same', data_format=data_format,
                activation=activation, bn=True)
        x = CBR(x, 32, 3, 2, training, momentum, mode, name='conv5', padding='same', data_format=data_format,
