...
class PbTester(object):
	...
	def __init__(self, param, data_root, image_height, image_width, batch_size, logger, pb_test_dir="./pb_test_result/"):
		tf.reset_default_graph()

		if not os.path.exists(pb_test_dir):
			os.makedirs(pb_test_dir)
		self.session = tf.Session()
		self.data_manager = DataManager(data_root, batch_size, image_height, image_width, False, False)
		self.pb_test_dir = pb_test_dir
		self.pb_file_path = param["pb_path"]
		...
		with self.session.as_default():
			...
			for batch in range(self.data_manager.num_batch):
				...
				for i in range(len(masks)):
					...
					filename = bytes.decode(path[i]).split('\\')[-1]
	...
	def test_decision(self):
		...
		with self.session.as_default():
			...
			pbar.close()
			time.sleep(0.1)
			print("Accuracy:{}, Average time per decision:{}".format(accuracy,
			                                                         total_time / num_step / self.data_manager.batch_size))
			self.logger.info("Accuracy:{}, Average time per decision:{}".format(accuracy,
			                                                                    total_time / num_step / self.data_manager.batch_size))
			self.logger.info(
				"False account: {}, path: {}, decision_out: {}".format(false_account, false_path, false_decision))
