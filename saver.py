...
class Saver(object):
	...
	def __init__(self, sess, checkpoint_dir, max_to_keep, input_nodes_list, output_nodes_list, pb_save_path, pb_name,
	             saving_mode, logger, model):
		"""
		...
		self.output_nodes = output_nodes_list
	...
	def save_pb(self):
		...
		if self.saving_mode == "CBR":
			...
			for key in source_list:
				if "moving_mean" in key:
					...
					key_var = key[0:-11] + "moving_variance"  # 11 is length of "moving_mean"
					...
					key_W = key[0:-18] + "CBR_Conv2D/kernel"  # 18 is length of "CBR_bn/moving_mean"
					...
					bias_name = key_W[0:-6] + 'bias:0'  # 6 is length of "kernel"
		...
		pbpath = os.path.join(self.pb_save_path, self.pb_name)
	...
	def simple_save(self):
		tf.saved_model.simple_save(self.session, './pbmodel', inputs={'my_input': self.model.image_input},
		                           outputs={'my_output': self.model.decision_out})
